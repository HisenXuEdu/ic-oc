...
# plt.plot((K.iloc[start:end, 1]*7000000-50)/1000, label='Ky', color='#d62728')
...
# # plt.plot(pose.iloc[start:end, 1], label='Y', color = 'g')
# # plt.plot(pose.iloc[start:end, 2], label='Z', color = 'b')
# plt.plot(pose.iloc[start:end, 0], label='X')
# plt.plot(pose.iloc[start:end, 1], label='Y')
# plt.plot(pose.iloc[start:end, 2]-17, label='Z')
# plt.ylabel('Pose(mm)', fontsize=14, labelpad=14)
# # 设置y轴间隔
# plt.yticks(range(-90, 20, 30))
# plt.ylim(-100, 20)
# plt.legend(loc='upper right')
...
# csv_data = pd.read_csv(csv_file)#防止弹出警告
# force = pd.DataFrame(csv_data)
# force = pose.iloc[start:end, [3,4,5]]

# x = range(0, len(force), 1)
# x = [i/4.8 for i in x]
