...
class DobotApi:
    def __init__(self, ip, port, *args):
        ...
        # if self.port == 29999 or self.port == 30003 or self.port == 30004 or self.port == 60000:
        if self.port in [29999, 30003, 30004, 30005, 30006, 60000]:
            try:
...
class DobotApiDHTool(DobotApi):
    ...
    def send_data(self, string):
        ...
        self.socket_dobot.send(bytes.fromhex(string))

    def generate_crc(self, hex_string):
        """
        根据前六位生成CRC
        """
        hex_string = hex_string.replace(" ", "")
        # 初始化CRC对象（使用CRC-16算法），生成多项式是0x8005
        crc_func = crcmod.predefined.Crc("modbus")
        # 将16进制字符串转换为字节串
        bytes_input = binascii.unhexlify(hex_string)

        # 计算CRC校验码并将结果转换为16进制字符串
        crc_func.update(bytes_input)
        crc_value = crc_func.crcValue
    ...
    def convert_to_hex(self, number):
        ...
        整数转化为四位16进制数
        """
        hex_number = hex(number)[2:].zfill(4)
        return hex_number
    
    def hex_to_num(self, string):
        """
        16进制数转化为整数
        """
        if string[3] == '6':
    ...
    def InitTool(self, is_total=False):
        """
        初始化夹爪
        is_total: 是否进行完全初始化
        Return:
        ...
        """
        if is_total:
            string = "01 06 01 00 00 A5"
        else:
            string = "01 06 01 00 00 01"
        string = string + " " + self.generate_crc(string)
        self.send_data(string)
        string = self.wait_reply()
        return self.hex_to_num(string)
    
    def SetToolForce(self, num):
        """
        设置力值
        Input:
        20-100 ，百分比
        ...
        读取当前设定力值
        """
        num = self.convert_to_hex(num)
        num = num[:2] + " " + num[2:]
        string = "01 06 01 01 " + " " + num
        string = string + " " + self.generate_crc(string)
        self.send_data(string)
        string = self.wait_reply()
        return self.hex_to_num(string)
    
    def GetToolForce(self):
        """
        读取当前设定力值
        Return:
        读取当前设定力值
        """
        string = "01 06 01 01 00 01"
        string = string + " " + self.generate_crc(string)
        self.send_data(string)
        string = self.wait_reply()
        return self.hex_to_num(string)
    
    def SetToolPos(self, num):
        """
        设定夹爪位置
        Input:
        0-1000 ，千分比
    ...
    def GetToolStatus(self):
        ...
        3 ：物体掉落
        """
        string = "01 03 02 01 00 01"
        string = string + " " + self.generate_crc(string)
        self.send_data(string)
        string = self.wait_reply()
        return self.hex_to_num(string)
    
    def GetRealToolPos(self):
        """
        反馈当前夹爪实时位置
        Return:
        读取当前实时位置
        """
        string = "01 03 02 02 00 01"
        string = string + " " + self.generate_crc(string)
        self.send_data(string)
        string = self.wait_reply()
        return self.hex_to_num(string)
    
    def SetInitDirection(self, num=0):
        """
        配置初始化方向
        Input:
        0 ：打开；
    ...
    # TODO:
    # def SetBaud(self, num=0):
    # def SetStopbit(self, num=1):
    # def SetCheckbit(self, num=0):
    # def TestIO(self, num):
    # def SetIOStatus(self, num):
    # def SetIO(self):
