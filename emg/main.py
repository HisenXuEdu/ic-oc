...
def cal():
    tracker = IMUTracker(sampling=144, data_order={'w': 2, 'a': 1, 'm': 3})
    dev_emg = pytrigno.TrignoIMU(channel_range=(0,0), samples_per_read=2000,
                    host='127.0.0.1')
    dev_emg.start()
    ...
    if 1==1:
        ...
        data = np.array(x)
        # 伪造磁场
        data[:, 6:] = np.array([0, 0, 50])
        print(data[400])

        print('initializing...')
        init_list = tracker.initialize(data[20:45])
        print(init_list)
        plt.plot(data[45:, 0:3])
        plt.show()
        ...
        # EKF step
        a_nav, orix, oriy, oriz = tracker.attitudeTrack(data[45:], init_list)
